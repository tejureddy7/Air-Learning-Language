...
class VecPyTorch(VecEnvWrapper):
    def __init__(self, venv, device):
        ...
        try:
            status = utils_1.get_status(model_dir)
        except OSError:
            status = {"num_frames": 0, "update": 0}
        #txt_logger.info("Training status loaded\n")
        #print("$$$$$$$$$$$", self.observation_space)
        obs_space, obs = self.get_obss_preprocessor(self.observation_space)
        if "vocab" in status:
            obs.vocab.load_vocab(status["vocab"])
    ...
    def get_obss_preprocessor(self, obs_space):
    ...
    def preprocess_texts(self, texts, vocab, device=None):
        ...
        for text in texts:
            ...
            #print("text, token", text, tokens, [vocab[token] for token in tokens])
