...
class RolloutStorage(object):
    def __init__(self, num_steps, num_processes, obs, action_space, recurrent_hidden_state_size):
        self.obs_space = {"image": obs.spaces["image"].shape, "text": 100}
        #self.vocab = Vocabulary(self.obs_space["text"])
        #print("roolout",num_steps, torch.zeros(num_steps + 1, num_processes, 5))
        self.obs = {"image": torch.zeros(num_steps + 1, num_processes, *obs.spaces["image"].shape), "text": torch.zeros(num_steps + 1, num_processes, 5)}
        #print(self.obs)
    ...
    def to(self, device):
        ...
        self.obs["image"] = self.obs["image"].to(device)
        self.obs["text"] = self.obs["text"].to(device)
    ...
    def insert(self, obs, recurrent_hidden_states, actions, action_log_probs, value_preds, rewards, masks):
        self.obs["image"][self.step + 1].copy_(obs["image"])
        self.obs["text"][self.step + 1].copy_(obs["text"])
    ...
    def after_update(self):
        self.obs["image"][0].copy_(self.obs["image"][-1])
        self.obs["text"][0].copy_(self.obs["text"][-1])
    ...
    def feed_forward_generator(self, advantages, num_mini_batch):
        ...
        for indices in sampler:
            obs_batch = {"image": self.obs["image"][:-1].view(-1, *self.obs["image"].size()[2:])[indices], "text": self.obs["text"][:-1].view(-1, *self.obs["text"].size()[2:])[indices] }
