    ...
    def _step(self, action):
        ...
        try:
            ...
            if distance < settings.success_distance_to_goal:
                ...
                reward = -50.0 # negative reward increased
                self.success = False
                #do = choice([1,2])
                # for i in range(4): #uncomment while training
                #     self.airgym.take_discrete_action(1) # uncomment while training
                # choice(choose)()
    ...
    def on_episode_start(self):
        ...
        if(settings.use_history==True):
            for i in range(settings.CNN_time_samples):
                self.ss_state[(i*settings.SS_input_size):((i+1)*settings.SS_input_size)] = self.airgym.get_SS_state(self) #we get the lidar sensor values here
        else:
            self.ss_state = self.airgym.get_SS_state(self) #this function - Obs state
